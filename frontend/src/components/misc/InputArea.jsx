 ...
 * 
 * @returns {JSX.Element} - Input area
 */

const InputArea = ({ required, type, placeholder, defaultValue, onChange, borderColor, autoComplete, onKeyDown, onKeyUp, value }) => {
    return (

        <div className={`border-4 ${borderColor === undefined ? 'border-light-orange dark:border-dark-orange' : borderColor} rounded-lg`}>
            <textarea className='py-3 grid-cols-2 rounded w-full px-4 text-black
                focus:outline-none'
                required={required}
                type={type}
