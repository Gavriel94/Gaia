...
// import { Tab, Tabs, RadioGroup, ButtonGroup, Button, Radio, FormGroup, InputGroup, NumericInput } from "@blueprintjs/core";
...
/**
 * taken from official Cardano documentation and altered to style with Gaia
 * 
 *  ! Deprecated and using logic from LoginButton
 */
...
export default class WalletConnect extends React.Component {
    ...
    //! this function enables the popup
    enableWallet = async () => {
        const walletKey = this.state.whichWalletSelected;
        try {
    ...
    refreshData = async () => {
        ...
        console.log(this.generateScriptAddress())
    ...
    /** !                                           !!!!!!!!!!!!!!!!!!!!!!!!!!
     * ! THESE FUNCTIONS HAVEN'T BEEN IMPLEMENTED
     * !
    /** !
                    ...
                    >
                        ...
                        {this.state.wallets.map(key =>
                            <div key={key} className='flex justify-center pt-2 min-w-full'>
                                <Button
                                    title={'Wallet'}
                                    func={() => this.state.whichWalletSelected}
                                    label={window.cardano[key].name}
                                    image={window.cardano[key].icon}
                                    imageAlt={'Wallet icon'}
                                    imageWidth={48}
                                    imageHeight={48}
                                    className='p-2 justify-center min-w-full'
                                />
                            </div>)}
                        <div className='flex justify-center pt-4 pb-4'>
                            <Button func={() => this.handleModal()} icon={<MdOutlineCancel />} />
                        </div>
                    </Modal>
                </div>
                {/* {
                    this.openDropdown && (this.state.wallets.map(key => 
                    <div key={key} className='bg-light-orange'>
                        <Button
                            title={'Wallet'}
                            func={this.state.whichWalletSelected}
                            image={window.cardano[key].icon}
